...
.DESCRIPTION
  ...
  This build writes ONLY to a QuickBooks sandbox and never modifies Gmail; the
  installer makes no accuracy or authority claim.
...
# company files found. ap-hub is QBO-sandbox-only, so this is DETECTION ONLY
# unless the operator explicitly enables the Desktop adapter (which trades away
# the sandbox-only guarantee).
