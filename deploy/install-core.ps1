...
.DESCRIPTION
  ...
  Provides:
    ...
    Get-ApHubPaths                -> resolved application paths
    ...
    Write-ApHubVerifiedRecovery   -> external recovery write + restore proof
    Invoke-ApHubInstall           -> run the real install, reporting via -OnProgress

  QBO sandbox remains the installer default. Production requires separate
  owner-controlled credentials, exact company binding, backup proof, and the
  default-off production gate. Gmail access may create drafts but never send
...
function Get-ApHubCredentialFields {
  ...
  @(
    ...
    New-CredField -Name "RECOVERY_TARGET" -Label "External recovery folder" -Required $true -Secret $false `
      -Group "Core" `
      -Help "A removable-drive or UNC/network folder. Setup writes and restores a recovery artifact here before it can finish."
    ...
    New-CredField -Name "QB_DESKTOP_TENANT_ID" -Label "AP Hub tenant ID for Desktop" -Required $false -Secret $false `
      -EnvNames @("QB_DESKTOP_TENANT_ID") -Group "QuickBooks Desktop (optional)" `
      -Help "Exact tenant bound to this Web Connector install. Required before enabling writes."
    New-CredField -Name "QB_DESKTOP_CONNECTION_ID" -Label "AP Hub Desktop connection ID" -Required $false -Secret $false `
      -EnvNames @("QB_DESKTOP_CONNECTION_ID") -Group "QuickBooks Desktop (optional)" `
      -Help "Exact active QBD connection bound to this install. Required before enabling writes."
...
function Get-ApHubEnvBody {
  ...
  $lines = @(
    ...
    "# QBO sandbox is the installer default; production requires separate owner-controlled setup.",
    ""
  )
  $order = @(
  ...
  if ($qbMode -eq "readonly") {
    ...
    $lines += @(
      ...
      "QB_DESKTOP_TENANT_ID=$([string]$Values['QB_DESKTOP_TENANT_ID'])",
      "QB_DESKTOP_CONNECTION_ID=$([string]$Values['QB_DESKTOP_CONNECTION_ID'])",
...
function Test-ApHubExternalRecoveryTarget {
  param([string]$Target)
  if ([string]::IsNullOrWhiteSpace($Target)) { return $false }
  if ($Target -match '^[\\]{2}') { return $true }
  try {
    $root = [IO.Path]::GetPathRoot([IO.Path]::GetFullPath($Target))
    return ([IO.DriveInfo]::new($root)).DriveType -eq [IO.DriveType]::Removable
  } catch { return $false }
}

function Write-ApHubVerifiedRecovery {
  param([hashtable]$Values, [string]$Target)
  if (-not (Test-ApHubExternalRecoveryTarget -Target $Target)) {
    throw "RECOVERY_TARGET must be a removable drive or UNC/network folder."
  }
  $folder = Join-Path $Target ("APHub-Recovery-{0}" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
  New-Item -ItemType Directory -Force -Path $folder | Out-Null
  $keyPath = Join-Path $folder "recovery.key"
  [IO.File]::WriteAllText($keyPath, (Get-ApHubRecoveryBody -Values $Values), (New-Object Text.UTF8Encoding($false)))
  $restored = Get-Content -Raw -LiteralPath $keyPath
  $match = [regex]::Match($restored, '(?m)^ENCRYPTION_KEY=(.+)$')
  if (-not $match.Success -or $match.Groups[1].Value.Trim() -ne [string]$Values["ENCRYPTION_KEY"]) {
    throw "External recovery restore verification failed."
  }
  $sha = [Security.Cryptography.SHA256]::Create()
  try {
    $hash = -join ($sha.ComputeHash([Text.Encoding]::UTF8.GetBytes($match.Groups[1].Value.Trim())) | ForEach-Object { $_.ToString("x2") })
  } finally { $sha.Dispose() }
  @{
  ...
  } | ConvertTo-Json | Set-Content -Encoding utf8 (Join-Path $folder "recovery-proof.json")
  return [pscustomobject]@{ KeyPath=$keyPath; Verified=$true }
}
...
.OUTPUTS
  [pscustomobject] Success / RecoveryKeyPath / RecoveryWasCreated / RecoveryVerified / AppUrl / Error
#>
function Invoke-ApHubInstall {
  ...
  $result = [pscustomobject]@{ Success = $false; RecoveryKeyPath = $null; RecoveryWasCreated = $false; RecoveryVerified = $false; AppUrl = $paths.AppUrl; HealthUrl = $paths.HealthUrl; QwcPath = $null; Error = $null }
  ...
  try {
    ...
    if (-not (Test-Path $paths.EnvPath)) {
      ...
      # Prove recovery before creating the authoritative .env. A failed target
      # cannot leave a partial install that later skips recovery as "existing".
      $recovery = Write-ApHubVerifiedRecovery -Values $Values -Target ([string]$Values["RECOVERY_TARGET"])
      $result.RecoveryKeyPath = $recovery.KeyPath
      $result.RecoveryWasCreated = $true
      $result.RecoveryVerified = $recovery.Verified
