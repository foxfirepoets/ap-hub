...
describe('the action surface is complete and symmetric', () => {
  it('registers a channel for every mutation, and 35 of them', () => {
    expect(ACTION_ENTRIES).toHaveLength(35);
...
describe('the role matrix matches the wrapper that actually gates each channel', () => {
  ...
  // Exhaustive: every channel × every non-owner role. 34 × 2 = 68 cells, none sampled.
  ...
  it('refuses bookkeeper on every owner-only channel, and there are 23 of them', async () => {
    const ownerOnly = ACTION_ENTRIES.filter((e) => !admits(e, 'bookkeeper'));
    expect(ownerOnly).toHaveLength(23);
  ...
  it('refuses cpa on all 34 channels that are not the notification read', async () => {
...
function surrogate(channel: string): Record<string, unknown> {
  switch (channel) {
    ...
    case 'aphub:backup:restore-external':
      return { restored: true, rowCounts: {} };
    case 'aphub:backup:export':
      return { exported: true };
    case 'aphub:backup:repair':
      return { repaired: true, migrationsApplied: 0, backupKeyPresent: true };
    default:
      return {};
  }
}
