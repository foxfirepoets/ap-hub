...
export default function TransactionDetailPage() {
  ...
  // SPIKE: useParams() returns the build-time sentinel value in a static export served for
  // an arbitrary runtime path, because Next bakes params into the exported HTML/RSC payload.
  // Reading the real id requires parsing the actual URL the shell loaded instead.
  const pathId = typeof window !== 'undefined' ? window.location.pathname.split('/').pop() : params.id;
  const id = Number(pathId);
  if (typeof window !== 'undefined') console.log('>>> SPIKE pathId =', pathId, 'params.id =', params.id);
