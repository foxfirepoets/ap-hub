...
3. .ralph/guardrails.md — must-not-cross lines (the six guarantees live here)
...
Database: PostgreSQL (raw pg + custom idempotent migrate runner — npm run migrate:up)
...
If your planned action violates a guardrail (the six guarantees, thin-client rule, tenant scope, audit_log, Do Not Build): stop, write the conflict to errors.log, emit:
...
- Do not add a QBO-write or Gmail-send path outside the existing guarded functions.
